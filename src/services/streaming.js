...
export class StreamingService {
    ...
    async switchTracksDynamic(message) {
    ...
    async playVideoFromQueueItem(message, queueItem, seekTimeMs = 0) {
        this.queueService.setPlaying(true);
        if (seekTimeMs === 0) {
            this.pausedTimeMs = 0;
        }
    ...
    setupStreamConfiguration(videoParams, userId) {
    ...
    async executeStream(inputForFfmpeg, streamOpts, message, title, videoSource, seekTimeMs = 0) {
        const userId = message.author.id;
    ...
    async prepareVideoSource(message, videoSource, title, seekTimeMs = 0, trackOverrides) {
        const mediaSource = await this.mediaService.resolveMediaSource(videoSource, trackOverrides?.audioStreamIndex, trackOverrides?.subtitleStreamIndex, seekTimeMs);
    ...
    async executeStreamWorkflow(input, options, message, title, source, seekTimeMs = 0) {
    ...
    async playVideo(message, videoSource, title, videoParams, seekTimeMs = 0, trackOverrides) {
        ...
        try {
            ...
            if (trackOverrides) {
                if (trackOverrides.audioStreamIndex !== undefined)
                    streamOpts.audioStreamIndex = trackOverrides.audioStreamIndex;
                if (trackOverrides.subtitleStreamIndex !== undefined)
                    streamOpts.subtitleStreamIndex = trackOverrides.subtitleStreamIndex;
            }
