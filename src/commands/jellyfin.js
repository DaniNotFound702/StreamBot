...
export default class JellyfinCommand extends BaseCommand {
    ...
    aliases = ["jf"];
    mediaService;
    static lastSearchResults = new Map();
    constructor() {
        super();
    ...
    async handlePlay(context) {
        ...
        try {
            let subsIndex = undefined;
            let audioIndex = undefined;
            let queryArgs = [...args];
    ...
    async applyStreamSelections(jellyfinItemId, userId, subsIndex, audioIndex) {
        try {
    ...
    async handleListLibraries(context) {
        try {
